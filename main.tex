...
\begin{document}
\title{IoT Middleware Benchmarking: A Scalable Architecture}
...
% in the abstract
\begin{abstract}
  With the rise in popularity of the Internet of Things in all kinds of different communication scenarios, several new middlewares have been developed with different use-cases and optimizations in mind, providing a user with a great many number to choose from in their own deployment. From this, stems the need to evaluate which middlewares are better at what, and in which use-cases. However, there exists no easy way to measure performance across different middlewares platforms that assures common ground between experiments, and that also permits a swift addition of any new platform that a user may want to consider in the future. In this paper, we will propose an architecture for publish/subscribe middlewares and develop and application based on it that addresses these issues, and show the benefits in time and effort that can be reaped from such an approach. We will validate the platform by testing different middlewares, and show the necessary changes that were required for each new addition and attempt to quantify the effort involved through lines of code and structure similarity. 
\end{abstract}
